...
create the vectors and map with no links     DONE

open given map       DONE

read row/row and insert that into adjacency     DONE
read col/coll and insert into adjacency      DONE

use bfs with parent vector instead of just singular parent    DOME

run that shit back to find all paths with diff parents     DONE

find shortest one in relation to turns       DONE

print it all out perfectly and chuck in some file i think            DONE

I recon have a phatty map class and just have all functions in that.         DONE

That way, can legit just had my robot class the map class later and its chilling so make everything public       DONE
...
public:
    ...
    void readMap() {
        ...
        if (myfile.is_open()) {
            ...
            for (int i = 0; i < 11; i++) {
                ...
                else if (i == 3) { offset = 9; }
                else if (i == 4) { offset = 9; }
                else if (i == 5) { offset = 18; }
                else if (i == 6) { offset = 18; }
                else if (i == 7) { offset = 27; }
                else if (i == 8) { offset = 27; }
                else if (i == 9) { offset = 36; }


                // EVEN LOGIC EVEN LOGIC EVEN LOGIC EVEN LOGIC EVEN LOGIC EVEN LOGIC
                //          gota check 3 7 11 15 19 22 27 31 35
                if (i % 2 == 0) {
                    if (i == 0 || i == 10) {                //we dont really care about these rows, 37 skips
                        for (int j = 0; j < 37; j++) {
                            charBuff = myfile.get();
                            //cout << charBuff << endl;
    ...
    void find_paths(int u) {  // give u end when calling first gota work back
        //CODE BASED OFF https://www.geeksforgeeks.org/print-all-shortest-paths-between-given-source-and-destination-in-an-undirected-graph/   CODE BASED OFF
        //CODE BASED OFF https://www.geeksforgeeks.org/print-all-shortest-paths-between-given-source-and-destination-in-an-undirected-graph/   CODE BASED OFF
        //CODE BASED OFF https://www.geeksforgeeks.org/print-all-shortest-paths-between-given-source-and-destination-in-an-undirected-graph/   CODE BASED OFF
    ...
    void findleastTurns() {
        ...
        for (auto vec : paths) {
            ...
            for (auto ele : pathsReversed) {
                ...
                if (prevEle == -1) { // we are checking out the first node, skip
                    //dont have to do anything
                }
                else if (prevPrevEle == -1) { // we are in second node so check dirrection    
                    if (prevEle == ele - 1 && startBearing != '>') { turns++; if (startBearing == '<') { turns++; } }       //do double turn if pointing complete wrong way
                    else if (prevEle == ele + 1 && startBearing != '<') { turns++; if (startBearing == '>') { turns++; } }
                    else if (prevEle == ele - 9 && startBearing != 'v') { turns++; if (startBearing == '^') { turns++; } }
                    else if (prevEle == ele + 9 && startBearing != '^') { turns++; if (startBearing == 'v') { turns++; } }
                }
                else { // now we are on atleast the third possition in the path so check all in same line
                    if (prevEle == prevPrevEle + 1 && ele != prevEle + 1) { turns++; }  //going right
                    else if (prevEle == prevPrevEle - 1 && ele != prevEle - 1) { turns++; }  //going left
                    else if (prevEle == prevPrevEle + 9 && ele != prevEle + 9) { turns++; }  //going down
                    else if (prevEle == prevPrevEle - 9 && ele != prevEle - 9) { turns++; }  //going up
                }
    ...
    void createPathPlan() {
        ...
        for (auto ele : pathsReversed) {
            ...
            if (prevEle == -1) { // we are checking out the first node, skip
                ...
                if (startBearing == '^') { pathPlanB += "N"; }
                if (startBearing == '>') { pathPlanB += "E"; }
                if (startBearing == '<') { pathPlanB += "W"; }
                if (startBearing == 'v') { pathPlanB += "S"; }
                prevPrevEle = prevEle;
                prevEle = ele;
                continue;
            }
            else if (prevPrevEle == -1) { // we are in second node so check dirrection of start    

                if (prevEle == ele - 1 && startBearing != '>') {  // we went right
                    if (startBearing == '^') { pathPlanB += "R"; steps++; }
                    else if (startBearing == '<') { pathPlanB += "LL"; steps++; steps++; }
                    else if (startBearing == 'v') { pathPlanB += "L"; steps++; }
                }

                else if (prevEle == ele + 1 && startBearing != '<') {
                    if (startBearing == '^') { pathPlanB += "L"; steps++; }
                    else if (startBearing == '>') { pathPlanB += "LL"; steps++; }
                    else if (startBearing == 'v') { pathPlanB += "R"; steps++; }
                }
                else if (prevEle == ele - 9 && startBearing != 'v') {
                    if (startBearing == '^') { pathPlanB += "LL"; steps++; steps++; }
                    else if (startBearing == '<') { pathPlanB += "L"; steps++; }
                    else if (startBearing == '>') { pathPlanB += "R"; steps++; }
                }
                else if (prevEle == ele + 9 && startBearing != '^') {
                    if (startBearing == 'v') { pathPlanB += "LL"; steps++; steps++; }
                    else if (startBearing == '<') { pathPlanB += "R"; steps++; }
                    else if (startBearing == '>') { pathPlanB += "L"; steps++; }
                }
                ...
                if (prevEle == prevPrevEle + 1 && ele != prevEle + 1) { // --> -->
                    if (prevEle == ele + 9) { pathPlanB += "L"; steps++; }
                    if (prevEle == ele - 9) { pathPlanB += "R"; steps++; }
                }
                ...
                else if (prevEle == prevPrevEle + 9 && ele != prevEle + 9) { // v v
                    if (prevEle == ele + 1) { pathPlanB += "R"; steps++; }
                    if (prevEle == ele - 1) { pathPlanB += "L"; steps++; }
                }
